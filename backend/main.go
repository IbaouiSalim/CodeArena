// Package main provides the HTTP API server for CodeArena
package main
...
// main initializes the executor, database, and HTTP server
func main() {
...
// executeHandler executes code in a Docker container (non-interactive)
...
// snippetsHandler saves code snippets to the database
...
// upgrader upgrades HTTP connections to WebSocket for interactive execution
...
// wsExecuteHandler handles interactive code execution over WebSocket
...
// snippetByTokenHandler retrieves a saved code snippet by its unique token
func snippetByTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Snippet retrieved: token=%s, lang=%s", snippet.Token, snippet.Language)
	w.Header().Set("Content-Type", "application/json")
