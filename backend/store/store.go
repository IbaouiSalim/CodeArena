// Package store persists code snippets to SQLite database
package store
...
// Store manages persistence of snippets in SQLite database
type Store struct {
	db *sql.DB
}

// New creates a Store and initializes the SQLite database schema
...
// Create saves a new snippet to the database and returns its unique token
...
// Get retrieves a snippet by token, returning nil if not found
...
// generateToken creates a random 12-character hexadecimal token
